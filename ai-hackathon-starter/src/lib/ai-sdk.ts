 ...
 */
export function itemsToLanguageV1Messages(model: LanguageModelV1, items: protocol.ModelItem[]): LanguageModelV1Message[] {
  ...
  for (const item of items) {
    if (item.type === 'message' || typeof item.type === 'undefined') {
      ...
      if (Array.isArray(currentAssistantMessage.content) && currentAssistantMessage.role === 'assistant') {
    ...
    if (item.type === 'reasoning' && item.content.length > 0 && typeof item.content[0]!.text === 'string') {
      messages.push({
        role: 'assistant',
        content: [
          {
 ...
 */
function handoffToLanguageV1Tool(model: LanguageModelV1, handoff: SerializedHandoff): LanguageModelV1FunctionTool {
  return {
    type: 'function',
 ...
 */
export function getResponseFormat(outputType: SerializedOutputType): LanguageModelV1CallOptions['responseFormat'] {
  if (outputType === 'text') {
    return {
      type: 'text'
    };
  ...
  async getResponse(request: ModelRequest) {
    return withGenerationSpan(async (span) => {
      try {
        ...
        const responseFormat: LanguageModelV1CallOptions['responseFormat'] = getResponseFormat(request.outputType);
        ...
        const response = {
          ...
          usage: new Usage({
            inputTokens: Number.isNaN(result.usage?.promptTokens) ? 0 : (result.usage?.promptTokens ?? 0),
            outputTokens: Number.isNaN(result.usage?.completionTokens) ? 0 : (result.usage?.completionTokens ?? 0),
            totalTokens:
              (Number.isNaN(result.usage?.promptTokens) ? 0 : (result.usage?.promptTokens ?? 0)) +
              (Number.isNaN(result.usage?.completionTokens) ? 0 : (result.usage?.completionTokens ?? 0))
          }),
          output,
          providerData: result
        } as const;
    ...
    try {
      ...
      for await (const part of stream) {
        ...
        switch (part.type) {
          ...
          case 'finish': {
            usagePromptTokens = Number.isNaN(part.usage?.promptTokens) ? 0 : (part.usage?.promptTokens ?? 0);
            usageCompletionTokens = Number.isNaN(part.usage?.completionTokens) ? 0 : (part.usage?.completionTokens ?? 0);
            break;
          }
          case 'error': {
            throw part.error;
